...
2. **市场和财务基本信息**：总市值、股本数、上市时间、市盈率、每股自由现金流、杜邦分析、经营投融资现金流净额、ROCE（已动用资本回报率）。ROCE 公式按模板计算：`EBIT = 净利润 + 利息费用 + 税费`，`ROCE = EBIT / (净资产 - 多余现金)`；多余现金需从 PDF 附注中查找并汇总一切现金的资金项目（一定是现金，比如定期活期存款、大额存单、理财产品等，不能及时变现的交易性金融资产和上下游账款不属于这一类）
3. **财务分析**：
...
1. **开头摘要区域**：
   - 本报告需要注意的重点分为三类：危险信号、机会信号、关注事项
   - 颜色规则：红=危险，绿=机会，黄=关注
   - 重要但并非缺陷或风险的问题（如一次性影响、结构性变化、大额分红、关联交易、海外扩张、客户集中度等）应归入「关注事项」，不要放入「危险信号」
   - 重点图表展示（盈利能力趋势、ROE 趋势、ROCE 趋势、资产负债结构、现金流走势等），其中 ROCE 必须在报告开头体现
   - 后续完整数据的目录导航

2. **正文区域**：
   - 将 Markdown 内容转为 HTML，保持表格和层级结构
   - 关键指标用颜色标注（红=危险，绿=机会，黄=关注）
   - 表格添加斑马纹样式
