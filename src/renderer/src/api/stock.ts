...
export const fetchConditionStockList = async (
  query: string,
  ...
  const list = (res.data.result.dataList as any[]).map<FilterItem>((item) => {
    ...
    const dividendKey = keys.find((key) => key.includes('DIVIDENDYIELD')) || '';
    ...
    return {
      ...
      pb: Number(item['PB']),
      peTtm: Number(item['PE_DYNAMIC']),
      dividend: Number(item[dividendKey]),
      ...item,
    };
  });
